...
def create_model(captcha_length, captcha_num_symbols, input_shape, model_depth=6, module_size=2):
...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            normalized_data = numpy.array(gray_data) / 255.0
            
            # kernel = numpy.ones((2, 2), numpy.uint8)
            # denoised_data = cv2.dilate(bw_data, kernel, iterations = 1)
            processed_data = normalized_data.reshape([self.captcha_height, self.captcha_width, 1])
